    ...
    StartLogic() {
        ...
        if(this.owner.passiveSkills)
        {
            for (let i = 0; i < this.owner.passiveSkills.length; ++i) {
                ...
                let config = Runtime.configs.GetSkillPassive(passiveSkillId, level);
        ...
        for (let i = 0; i < this.owner.beHitList.length; ++i) {
            ...
            if(hitData.hitConfig.Affect?.length > 0){
                for(let j = 0; j < hitData.hitConfig.Affect.length; ++j){
                    ...
                    let buffCfg = Runtime.configs.Get("buff")[affect.Id]
                    if(buffCfg)
                    {
            ...
            if(hitData.ratio == 0 || battleDamageInfo != 0)
            {
                ...
                for(let j = 0; j < hitConfig.HitEffect.length; ++j) {
                    const data = JSON.parse(JSON.stringify(Runtime.configs.Get("effect")[hitConfig.HitEffect[j].Id]));
                    if(!data) continue;
                ...
                for (let j = 0; j < hitConfig.HitShake.length; ++j) {
                    let shakeData = JSON.parse(JSON.stringify(Runtime.configs.Get("shake")[hitConfig.HitShake[j].Id]));
                    if (!shakeData) return;
                ...
                for (let j = 0; j < hitConfig.HitSound.length; ++j) {
                    let soundData = JSON.parse(JSON.stringify(Runtime.configs.Get("sound")[hitConfig.HitSound[j].Id]));
                    if (!soundData) return;

...
export enum FrameType {
 ...
 export class Action {
    ...
    handleDamageBox(data) {
      let boxInfo = JSON.parse(JSON.stringify(Runtime.configs.Get("box")[data.Id]));
      if (!boxInfo) return;
    ...
    handleEffect(data) {
      let effectData = JSON.parse(JSON.stringify(Runtime.configs.Get("effect")[data.Id]));
    ...
    handleShake(data) {
      let shakeData = JSON.parse(JSON.stringify(Runtime.configs.Get("shake")[data.Id]));
      if (!shakeData) return;
    ...
    handleBullet(data) {
      const bulletData = JSON.parse(JSON.stringify(Runtime.configs.Get("bullet")[data.Id]));
    ...
    handleSound(data) {
      ...
      if (data.Id < 0) {
        ...
        soundData = JSON.parse(JSON.stringify(Runtime.configs.Get("sound")[data.Id]));
      if (!soundData) return;

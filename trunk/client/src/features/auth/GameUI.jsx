...
const GameUI = ({ theme, buildings = defaultBuildings, onPlace }) => {
                  ...
                  {list.map((item) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex-shrink-0 flex flex-col items-center gap-2 cursor-pointer transition-all hover:scale-105 ${
                          isSelected ? 'opacity-100' : 'opacity-80 hover:opacity-100'
                        }`}
                      >
                        {/* Decorative corner elements */}
                        <div className="relative">
                          <div className={`w-16 h-16 rounded-lg border-2 flex items-center justify-center transition-all backdrop-blur-sm ${
                            isSelected 
                              ? 'border-cyan-400 bg-gradient-to-br from-cyan-500/30 to-purple-500/20 shadow-lg shadow-cyan-500/50' 
                              : 'border-purple-500/40 bg-gradient-to-br from-purple-500/10 to-black/50 hover:border-purple-400/60 hover:shadow-md hover:shadow-purple-500/30'
                          }`}>
                            <Icon size={28} className={isSelected ? 'text-cyan-300' : 'text-purple-300/80'} />
                          </div>
                          {/* Corner decorations */}
                          {isSelected && (
                            <>
                              <div className="absolute -top-1 -left-1 w-2 h-2 border-l-2 border-t-2 border-cyan-400"></div>
                        ...
                        <div className={`flex items-center gap-1.5 px-2 py-1 rounded border transition-all ${
                          ...
                          <p className={`text-xs font-medium ${isSelected ? 'text-cyan-200' : 'text-white/90'}`}>
                            {item.name}
                          </p>
                          <p className={`text-xs font-semibold ${isSelected ? 'text-cyan-400' : 'text-purple-400'}`}>
                            ×{item.count}
                          </p>
                        </div>
                      </div>

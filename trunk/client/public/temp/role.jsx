...
export default function SciFiGameInterface() {
  const [activeTab, setActiveTab] = useState('装备');
  const [activeNav, setActiveNav] = useState('角色');
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#03060a] text-cyan-50 font-sans overflow-hidden select-none max-w-md mx-auto border-x border-cyan-900/30 shadow-2xl relative">
      ...
      {/* ================= 顶部信息栏 ================= */}
      <div className="flex items-center justify-between px-2 py-2 bg-slate-900/90 backdrop-blur-md border-b border-cyan-500/20 z-20 shrink-0 gap-2">
        {/* 头像与等级 */}
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative">
            <div className="w-11 h-11 rounded-full border-2 border-orange-500 bg-slate-800 overflow-hidden relative shadow-[0_0_10px_rgba(249,115,22,0.4)]">
              <img src="/api/placeholder/44/44" alt="Avatar" className="w-full h-full object-cover" />
            </div>
          ...
          <div className="flex flex-col">
            <span className="text-sm font-bold text-white tracking-wide shadow-black drop-shadow-md">龙阳焱</span>
            {/* 经验条 */}
            <div className="w-20 h-1.5 bg-slate-800 mt-1 rounded-full overflow-hidden">
              <div className="h-full w-[65%] bg-gradient-to-r from-green-400 to-emerald-600"></div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-1 justify-end gap-1.5 overflow-hidden">
          <ResourcePill icon={Diamond} value="2032" color="text-cyan-300" />
          <ResourcePill icon={Coins} value="50519" color="text-yellow-400" />
          <ResourcePill icon={Drumstick} value="6106/30" color="text-red-400" />
        </div>
      </div>
      ...
      <div className="relative flex-1 flex flex-col min-h-[380px] z-10 overflow-hidden">
        ...
        <div className="flex flex-1 relative p-1">
          ...
          <div className="flex-1 flex flex-col items-center justify-end pb-2 relative z-10">
            ...
            <div className="relative z-20 mt-auto mb-2">
               <div className="relative bg-gradient-to-r from-orange-600/90 to-red-600/90 px-6 py-1.5 rounded-full border border-orange-400 shadow-[0_0_15px_rgba(234,88,12,0.4)] flex items-center gap-2 transform skew-x-[-10deg]">
                  ...
                  <span className="text-2xl font-black text-white italic tracking-wider transform skew-x-[10deg]">1057</span>
          ...
          <div className="w-40 pt-1 z-20 flex flex-col items-end shrink-0 relative">
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
              <EquipSlot tier="2阶" level="32级" icon="helmet" rarity="purple" />
              <EquipSlot tier="2阶" level="30级" icon="glove" rarity="purple" />
              <EquipSlot tier="4阶" level="31级" icon="armor" rarity="gold" />
              <EquipSlot tier="3阶" level="31级" icon="pants" rarity="gold" />
              <EquipSlot tier="2阶" level="31级" icon="boots" rarity="purple" />
              <EquipSlot tier="2阶" level="30级" icon="glove" rarity="purple" />
            </div>
          </div>
        </div>
        ...
        <div className="h-11 mx-2 mb-2 bg-slate-900/80 border border-slate-700 rounded-lg flex items-center justify-between px-2 shrink-0 relative overflow-hidden backdrop-blur-sm">
          <div className="flex items-center gap-1.5 z-10">
             <div className="w-6 h-6 bg-orange-500 text-white text-xs font-bold flex items-center justify-center rounded shadow-md">1</div>
             ...
             <div className="flex gap-1 ml-2">
                {[2, 3, 4, 5].map(n => (
                  <div key={n} className="w-6 h-6 bg-slate-800/50 border border-slate-600 text-[10px] flex items-center justify-center text-slate-500 rounded hover:border-cyan-500 cursor-pointer">
                    {n}
                  </div>
                ))}
             </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 custom-scrollbar bg-black/20">
          <div className="grid grid-cols-6 gap-1.5">
            {inventoryItems.map((item, idx) => (
              ...
              >
                <div className="absolute top-0.5 left-0.5 text-[8px] text-slate-400 font-mono leading-none">{item.tier}阶</div>
        ...
        <div className="flex bg-[#050b14] border-t border-slate-800 shrink-0">
          {['宝石', '装备', '材料', '芯片'].map((tab) => (
            <button 
              ...
              className={`flex-1 py-3 text-xs font-bold text-center relative transition-colors ${activeTab === tab ? 'text-white bg-gradient-to-t from-orange-900/20 to-transparent' : 'text-slate-500 hover:text-slate-300'}`}
            >
              {tab}
              {activeTab === tab && (
                <motion.div 
      ...
      {/* ================= 底部导航 ================= */}
      <div className="bg-black/95 border-t border-slate-800 px-1 pb-2 pt-2 shrink-0 z-30">
        <div className="flex justify-between items-end">
          {[
            { id: 'shop', name: '商城', icon: ShoppingCart },
            { id: 'character', name: '角色', icon: User, special: true }, // 特殊高亮
            { id: 'core', name: '核心', icon: Cpu },
            { id: 'battle', name: '战斗', icon: Swords },
            { id: 'base', name: '基地', icon: Warehouse },
            { id: 'legion', name: '军团', icon: Shield },
            ...
            return (
              ...
              >
                ...
                <div className={`relative p-1.5 transition-all duration-300 ${isActive ? '-translate-y-1' : ''}`}>
                  ...
                  <item.icon 
                    size={isActive ? 22 : 18} 
                    className={`${isActive ? 'text-orange-400 drop-shadow-[0_0_5px_rgba(249,115,22,0.8)]' : 'text-slate-500'}`} 
                    strokeWidth={isActive ? 2.5 : 2}
                  />
                </div>
                <span className={`text-[9px] font-bold tracking-tight transition-all ${isActive ? 'text-orange-400' : 'text-slate-600'}`}>
                  {item.name}
                </span>
              </motion.div>
            );
...
function LeftSideButton({ icon: Icon, label, color }) {
  ...
  return (
    <div className="group relative flex flex-col items-center gap-0.5 cursor-pointer w-full">
      ...
      <span className={`text-[9px] font-bold scale-90 ${isYellow ? 'text-yellow-500' : 'text-slate-500 group-hover:text-orange-400'}`}>
        {label}
      </span>
      {/* 红点提示 */}
...
function EquipSlot({ tier, level, icon, rarity }) {
  ...
  return (
    <div className={`aspect-square rounded-lg border-2 ${colors[rarity]} relative flex items-center justify-center cursor-pointer overflow-hidden group hover:brightness-125 transition-all`}>
      {/* 左上角阶数 */}
...
function ItemIcon({ type, className }) {
  switch (type) {
    case 'helmet': return <Layers className={className} />;
    case 'armor': return <Shield className={className} />;
    case 'boots': return <Box className={className} />; // Placeholder shape
    case 'glove': return <Hexagon className={className} />; // Placeholder shape
    case 'pants': return <Layers className={`${className} rotate-180`} />;
